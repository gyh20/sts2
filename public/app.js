...
function renderHome(){
  layout(`<div class="grid two"><div class="card"><h2>房间</h2><div class="row"><button class="btn" id="create">创建房间</button><input id="rid" placeholder="输入房间 ID"><button class="btn secondary" id="join">加入</button></div><p class="muted small">房间链接形如 ${location.origin}${BASE}/room/ABC123</p></div><div class="card"><h2>游戏规则与提交说明</h2><div class="rule-block"><h3>共同规则</h3><ul><li>进入房间后先查看本局给定的种子、进阶和角色；开局时必须完全按这些设置填写。</li><li>使用非给定种子、错误进阶或错误角色完成的存档，不会计入本局成绩。</li><li>房主点击“开始游戏”后，种子、任务、模式和胜利条件会锁定。</li></ul></div><div class="rule-block"><h3>普通模式</h3><ul><li>普通模式的目标是“通关给定种子”。每个种子对应棋盘上的一个格子。</li><li>提交某个给定种子的胜利存档后，该队伍会完成/占领这个种子格。</li><li>率先完成房主设置的 k 个种子的队伍获胜；如果开启 Lockout，已被其他队伍占领的种子不能再被抢占。</li></ul></div><div class="rule-block"><h3>任务模式</h3><ul><li>任务模式会生成 5×5 任务棋盘，仍然只能使用房间给定的种子进行挑战。</li><li>每次提交都会自动检查这个存档完成了哪些任务，例如限时通关、金币/遗物数量、禁抓某类牌、卡牌收集等。</li><li>目标是完成房主设置的线数；同一队伍的累计类任务可以通过多个给定种子的提交逐步推进。</li></ul></div><div class="rule-block"><h3>提交方式</h3><ul><li>完成一局后，在本地选择 STS2 存档文件并提交评测，推荐上传 current_run.save 或 history/*.run。</li><li>如果文件上传不方便，也可以把存档 JSON 内容粘贴到提交框里。</li><li>提交成功后会生成评测记录，页面会显示通过原因和完成的格子；同一玩家两次提交至少间隔 5 分钟。</li></ul></div><div class="row"><button class="btn secondary" id="taskListHome">查看全部任务 / 留建议</button>${state.user?.isRoot?`<button class="btn secondary" id="adminHome">进入 root 后台</button>`:''}</div></div></div>`);
